function regex({ raw: texts }: TemplateStringsArray, ...insertions: RegExp[]) {
...
export const TEXT_PRESENTATION_SEQUENCE = regex`${EMOJI_CHARACTER}${TEXT_PRESENTATION_SELECTOR}`()

export const EMOJI_PRESENTATION_SELECTOR = /\u{FE0F}/u
export const EMOJI_PRESENTATION_SEQUENCE = regex`${EMOJI_CHARACTER}${EMOJI_PRESENTATION_SELECTOR}`()
...
export const EMOJI_MODIFIER_SEQUENCE = regex`${EMOJI_MODIFIER_BASE}${EMOJI_MODIFIER}`()

// 1.4.5 Emoji Sequences
export const REGIONAL_INDICATOR = /\p{Regional_Indicator}/u
export const EMOJI_FLAG_SEQUENCE = regex`${REGIONAL_INDICATOR}${REGIONAL_INDICATOR}`()

export const TAG_BASE = regex`${EMOJI_PRESENTATION_SEQUENCE}|${EMOJI_MODIFIER_SEQUENCE}|${EMOJI_CHARACTER}`()
...
export const EMOJI_CORE_SEQUENCE = 
  regex`${EMOJI_FLAG_SEQUENCE}|${EMOJI_MODIFIER_SEQUENCE}|${EMOJI_KEYCAP_SEQUENCE}|${EMOJI_PRESENTATION_SEQUENCE}|${EMOJI_CHARACTER}`()

export const EMOJI_ZWJ_ELEMENT = regex`${EMOJI_MODIFIER_SEQUENCE}${EMOJI_PRESENTATION_SEQUENCE}|${EMOJI_CHARACTER}`()
